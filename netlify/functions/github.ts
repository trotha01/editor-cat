...
/**
 * Filing an issue on this deployment's repository, from the in-app assistant.
 *
 ...
 *
 * Nothing here decides *whether* to file. The browser posts a draft the user has
 * seen, edited if they wanted to and pressed a button on — the assistant that
 * helped write it cannot reach this endpoint on its own. That is what keeps a
 * confused model, or a prompt-injected one, from being able to post anything at
 * all.
 *
...
/**
 * Whether the feature is available, answered without a session.
 *
 * The UI asks this before it offers to file anything, so that "report a bug"
 * either works or is never offered — an assistant that collects a careful
 * report and then discovers there is nowhere to send it has wasted the one
 * person willing to write one.
 *
 * Unauthenticated because of what it discloses: a boolean, and the name of the
 * public repository whose issues this site files. Both are on the screen of any
 * deployment where the feature works.
 */
...
export default async (request: Request): Promise<Response> => {
  ...
  if (route === 'status' && request.method === 'GET') return status()
