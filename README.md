...
applied; **`0003`, `0004`, `0006`, `0007`, `0008` and `0009` are outstanding**. All
six need running, and the order between them does not matter: `0004` only drops a
...
## The word shelf: an account row and a folder tree

The [word pages](#what-it-does) keep two things in step. The shelf itself — the
tiers, the languages under them, the words under those, and the order, labels and
transcripts of every take — is one row on your account. The videos are files in
the Drive folder you chose, in the layout anybody would build by hand:
...
**A folder cannot hold an order, so the account does.** `word_shelves` is one row
per person: the three lists as one JSON document, guarded by a version the same
way a project is (`src/lib/supabase/shelf.ts`,
`supabase/migrations/0009_word_shelf.sql`). It is written a beat after the last
edit rather than on every keystroke, and read whole — so opening the page on any
machine is one query, and the folder tree does not have to be walked to find out
what is on the shelf.

This used to be a file. Every word folder held an `editor-cat.json` listing that
word's takes with their labels and transcripts, which worked and cost two things:
a file in every folder you open on your phone, and a listing plus a download _per
word_ before the page could draw — sixty-odd round trips to Drive for a shelf of
twenty-three words. Nothing writes one now. They are still read, exactly once per
account: an account with no shelf yet is built out of the folder tree and
whatever those files still say (`readShelf`, `parseSidecar`), and then written up.
Old `editor-cat.json` files are left where they are rather than trashed — they are
in your Drive, and deleting somebody's files to tidy up is not this app's call.
...
- **The shelf needs an account; the videos do not.** Signed out, or on a
  ...
  browser's alone, saved in IndexedDB, and the videos still go to Drive. What you
