...
2. Run the files in `supabase/migrations/` in order — paste them into the
   ...
   writes into, so a sign-in restores it instead of asking for it again.
...
applied; **`0003`, `0004`, `0006` and `0007` are outstanding**. All four need
running, and the order between them does not matter: `0004` only drops a table,
`0007` only creates one, and `0006` repeats `0003`'s two
