...
applied; **`0003`, `0004`, `0006`, `0007`, `0008`, `0009`, `0010`, `0011` and
`0012` are outstanding**. Order matters at the end — `0011` drops what `0010`
replaces, so it goes after both, and `0012` rewrites the policies `0006` and
`0009` create, so it goes after those. Between the rest it does not: `0004` only
drops a
...
**Share** in the word page's header invites somebody onto your shelf by email
address. They sign in with Google exactly as you do — it is still the only way
in — and your shelf appears in a picker beside their own, with everything on it:
the tiers, the languages, the words, the takes and their transcripts. They can
add and change all of it. Nothing is emailed from here, so tell them it is
waiting.

**An invitation names an address; a share names a subject.** Every policy in this
schema matches on an Auth0 subject (`google-oauth2|104372…`), and nothing in this
deployment maps an address to one — Supabase holds no user table, and asking the
tenant would mean a Management API credential in the browser. So the row is
written with the address and no subject, and the person fills the subject in
themselves the first time they open the page. Until then the owner's list shows
them as **invited** rather than **joined**, which is also how a mistyped address
becomes findable. The claim requires a verified address, which for a Google
connection is always true.

**Only the shelf.** Your projects and timelines are not shared and the policy on
`projects` is untouched. What does go with it, and is worth knowing before you
invite anyone:

- They can **delete** things. It is one shelf with two people editing, not a copy
  each — that is the point — and the version guard that already kept two of your
  own machines from overwriting each other does the same job here.
- They can reach the **stored files** behind your generated media, in both
  directions. `assets` rows carry no link back to a shelf, so "the takes on this
  shelf" is not a question the schema can ask, and read is granted across the
  catalogue between two people who share one. It is not a listing — the library
  on the Image tab is still filtered to your own rows — but a known key or id
  resolves. Narrowing it would mean a join table of every take on every shelf,
  maintained on every save, and a new way for the shelf and the catalogue to fall
  out of step.
- They can **spend the deployment's generation budget**, the same as any signed-in
  visitor. That is true of anyone you let sign in at all; it is repeated here
  because inviting somebody is the one moment you are choosing to let one more
  person in.

**Revoking is deleting the row**, and it is immediate in the database. Storage
lags it by up to thirty seconds — `/api/r2/downloads` caches who shares with whom
for that long, which is what keeps opening a shelf from being one lookup per
batch of takes — and any presigned URL already issued lives out its own expiry.

**Takes belong to whoever filmed them.** Everyone's uploads go to their own
storage prefix and their own asset rows, whichever shelf they are filing into:
there is one ingest hook for the whole app and it has no notion of which page is
open. The share is what makes them reachable from the other side. The consequence
worth naming is that **leaving or being removed from a shelf takes your takes'
files with you** — the shelf still lists them, and the bytes stop resolving for
everybody else.

**Switching shelves clears this browser's copy.** The three IndexedDB stores hold
one shelf between them and say nothing about whose it is, so opening another
means clearing rather than merging — `mergeRemoteShelf` keeps local rows the
account has not heard about, which across a switch would graft one person's tiers
onto another's and write the result back to them. Anything not yet saved goes up
first. The videos themselves stay, since they are keyed by asset id and no shelf
owns them.

**How it is enforced**, since it crosses three boundaries and only two of them
are the database:

| Layer                    | What decides                                                                                                                                                                                                                             |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `word_shelves`           | `own_word_shelf` gains `public.shelf_member_of(user_id)` — directed, so being let onto somebody's shelf does not let them onto yours.                                                                                                    |
| `assets`                 | `own_assets` gains `public.shelf_partner(user_id)` on **read only**; the with-check is unchanged, so nobody writes a row under another subject.                                                                                          |
| R2 (`/api/r2/downloads`) | No row-level security to fall back on, so the prefix set is still derived from verified subjects rather than sent. A key under your own prefix is settled without asking anyone; anything else sends the function to the database first. |

That last lookup is why the browser sends **two tokens** to `/api/r2/downloads`:
the access token in `authorization`, which proves who is asking, and the Auth0 ID
token in `x-supabase-authorization`, which is the one Supabase accepts. The
function asks PostgREST as the caller rather than holding a service-role key —
the lookup is one the caller is entitled to make themselves — and refuses an ID
token whose subject is not the one the access token proved. Neither check is
sufficient alone: a forged token fails at PostgREST, and a genuine token
belonging to somebody else fails the comparison and is never sent.
`netlify/lib/shelfShares.ts` and `supabase/migrations/0012_shelf_shares.sql` are
